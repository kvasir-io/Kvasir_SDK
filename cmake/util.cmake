...
function(add_clean_file target file)
  get_target_property(cur_additional_clean_files ${target}
...
function(print_size target linker_file)
  add_custom_command(
    TARGET ${target}
    POST_BUILD
    COMMAND ${CMAKE_SIZE} -x --format=sysv
            "${CMAKE_CURRENT_BINARY_DIR}/${target}.elf"
    COMMAND
      ${Python3_EXECUTABLE} ${kvasir_cmake_dir}/tools/pretty_size.py "${CMAKE_SIZE}"
      "${CMAKE_CURRENT_BINARY_DIR}/${target}.elf" "${TARGET_FLASH_SIZE}"
      "${TARGET_RAM_SIZE}" "${TARGET_EEPROM_SIZE}" "${linker_file}")
endfunction()

function(generate_object target suffix type)

  list(TRANSFORM TARGET_EXTRA_FLASH_SECTIONS
       PREPEND "--only-section=" OUTPUT_VARIABLE extra_flash_sections)

  add_custom_command(
    TARGET ${target}
    POST_BUILD
    COMMAND
      ${CMAKE_OBJCOPY} --output-target ${type} --only-section=.data
      --only-section=.text ${extra_flash_sections}
      "${CMAKE_CURRENT_BINARY_DIR}/${target}.elf"
  ...
  add_custom_command(
    ...
    POST_BUILD
    COMMAND
      ${CMAKE_OBJCOPY} --output-target ${type} --only-section=.eeprom
      "${CMAKE_CURRENT_BINARY_DIR}/${target}.elf"
  ...
  if(${type} MATCHES ihex)
    add_custom_command(
      ...
      COMMAND
        ${CMAKE_OBJCOPY} --output-target ${type} --only-section=.data
        --only-section=.text --only-section=.eeprom ${extra_flash_sections}
        "${CMAKE_CURRENT_BINARY_DIR}/${target}.elf"
    ...
    add_custom_command(
      ...
      COMMAND
        ${Python3_EXECUTABLE} ${kvasir_cmake_dir}/tools/ihex_to_uf2.py
        "${CMAKE_CURRENT_BINARY_DIR}/${target}_eeprom_flash${suffix}"
        "${CMAKE_CURRENT_BINARY_DIR}/${target}_eeprom_flash.uf2"
        ${TARGET_UF2_CODE}
...
function(generate_lst target)
  list(TRANSFORM TARGET_EXTRA_FLASH_SECTIONS
       PREPEND "--section=" OUTPUT_VARIABLE extra_flash_sections)
  add_custom_command(
    TARGET ${target}
    POST_BUILD
    COMMAND
      ${CMAKE_OBJDUMP} --section=.text --section=.data ${extra_flash_sections}
      --disassemble --demangle --all-headers
      "${CMAKE_CURRENT_BINARY_DIR}/${target}.elf" >
...
function(add_bootloader_app_data target application)
  target_include_directories(
    ${target} PRIVATE ${CMAKE_CURRENT_BINARY_DIR}/generated/${target})
  ...
  add_custom_command(
    ...
    COMMAND
      ...
      ${CMAKE_CURRENT_BINARY_DIR}/generated/${target}/bootloader/app_data.inl
      DEPENDS ${kvasir_cmake_dir}/tools/gen_app_data_for_bootloader.py
    BYPRODUCTS
      ${CMAKE_CURRENT_BINARY_DIR}/generated/${target}/bootloader/app_data.inl)
...
function(
  ...
  target_include_directories(${name}
                             PUBLIC ${kvasir_cmake_dir}/../../chip/core/src)
  ...
  add_target_linker_dependency(${name}
                               ${kvasir_cmake_dir}/../linker/common_flash.ld)
  add_target_linker_dependency(${name}
                               ${kvasir_cmake_dir}/../linker/common_ram.ld)
  add_target_linker_dependency(${name}
                               ${kvasir_cmake_dir}/../linker/common_eeprom.ld)

  add_target_linker_dependency(${name} ${linker_file})
  add_target_linker_dependency(${name}
                               ${kvasir_cmake_dir}/tools/two_stage_link.py)
  ...
  if(CMAKE_CROSSCOMPILING)
    ...
    set_property(TARGET ${name} APPEND PROPERTY SOURCES ${CHIP_SOURCES})

    target_compile_options(${name} PUBLIC ${optimize_flags} ${sanitize_flags} ${CHIP_OPTIONS})

    foreach(current_lib ${libraries})
      target_compile_options(${current_lib} PUBLIC ${optimize_flags}
                                                   ${sanitize_flags})
    ...
    foreach(current_linker_flag ${linker_flags})
      if(${used_specs} STREQUAL ${SPEC_REPLACEMENT_EMPTY_MARKER})
        string(REPLACE ${SPEC_REPLACEMENT_STRING} "" current_linker_flag
                       ${current_linker_flag})
      else()
        string(REPLACE ${SPEC_REPLACEMENT_STRING} ${used_specs}
                       current_linker_flag ${current_linker_flag})
      endif()
...
function(target_configure_kvasir target)
  ...
  if(PARSED_ARGS_ENABLE_SELF_OVERRIDE AND NOT PARSED_ARGS_APPLICATION)
    message(
      FATAL_ERROR
        "${target}: cant enable self override for an normal application")
  endif()
  ...
  if(NOT PARSED_ARGS_APPLICATION)
    ...
    if(PARSED_ARGS_BOOTLOADER_SIZE)
      message(
        FATAL_ERROR
          "${target}: do not set BOOTLOADER_SIZE in bootloader instead set in application"
      )
    endif()
  endif()
  ...
  if(NOT PARSED_ARGS_BOOTLOADER)
    ...
    set_target_properties(${target} PROPERTIES bootloader_size
                                               ${PARSED_ARGS_BOOTLOADER_SIZE})
  endif()

  if(PARSED_ARGS_LINKER_FILE AND PARSED_ARGS_LINKER_FILE_TEMPLATE)
    message(
      FATAL_ERROR "${target}: only LINKER_FILE or LINKER_FILE_TEMPLATE allowed")
  endif()

  if(NOT ${PARSED_ARGS_APPLICATION} STREQUAL "FALSE")
    get_target_property(bootloader_size ${PARSED_ARGS_APPLICATION}
                        bootloader_size)
  else()
  ...
  if(NOT PARSED_ARGS_LINKER_FILE)
    if(PARSED_ARGS_LINKER_FILE_TEMPLATE)
      ...
      configure_file(
        ${CMAKE_CURRENT_SOURCE_DIR}/${PARSED_ARGS_LINKER_FILE_TEMPLATE}
        ${CMAKE_CURRENT_BINARY_DIR}/generated/${target}/generated.ld @ONLY)
      set(PARSED_ARGS_LINKER_FILE
          ${CMAKE_CURRENT_BINARY_DIR}/generated/${target}/generated.ld)
    else()
      set(PARSED_ARGS_LINKER_FILE ${LINKER_FILE})
    endif()
  else()
    set(PARSED_ARGS_LINKER_FILE
        ${CMAKE_CURRENT_SOURCE_DIR}/${PARSED_ARGS_LINKER_FILE})
  endif()
  ...
  if(NOT ${PARSED_ARGS_APPLICATION} STREQUAL "FALSE")
    target_compile_definitions(
      ${target}
      PUBLIC INPUT_ENABLE_SELF_OVERRIDE=${PARSED_ARGS_ENABLE_SELF_OVERRIDE})
    target_compile_definitions(${target}
                               PUBLIC INPUT_BOOTLOADER_SIZE=${bootloader_size})
  endif()
