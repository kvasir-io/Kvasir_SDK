...
namespace Kvasir { namespace Io { namespace Detail {
    ...
    template<typename T>
    struct IsPinLocation : FalseType {};

    template<int Port, int Pin>
    struct IsPinLocation<Register::PinLocation<Port, Pin>> : TrueType {};
    ...
    template<int LPort,
             ...
             int RPort,
             int RPin>
    constexpr bool PinLocationEqual(Register::PinLocation<LPort,
                                                          LPin> l,
                                    Register::PinLocation<RPort,
                                                          RPin> r) {
        return !Io::Detail::PinLocationLess<std::decay_t<decltype(l)>,
                                            std::decay_t<decltype(r)>>::value
            && !Io::Detail::PinLocationLess<std::decay_t<decltype(r)>,
                                            std::decay_t<decltype(l)>>::value;
    }

    template<typename T,
             typename U>
    constexpr bool PinLocationEqual(T,
                                    U) {
        return false;
    }
