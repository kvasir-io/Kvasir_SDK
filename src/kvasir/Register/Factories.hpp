...
namespace Kvasir { namespace Register {
    namespace Detail {
        ...
        struct Write<FieldLocation<TAddress, Mask, Access, TFieldType>, Value>
          : Action<
              FieldLocation<TAddress, Mask, Access, TFieldType>,
        ...
        struct ResetImpl<FieldLocation<TAddress, Mask, Access, TFieldType>> {
            ...
            static_assert(
              Detail::IsSetToClear<FieldLocation<TAddress, Mask, Access, TFieldType>>::value,
    ...
    template<typename T>
    constexpr MPL::EnableIfT<Detail::IsFieldLocation<T>::value, Detail::SetT<T>> set(T) {
        return {};
    }
    ...
    template<typename T>
    constexpr MPL::EnableIfT<Detail::IsFieldLocation<T>::value, Detail::ClearT<T>> clear(T) {
        return {};
    }
    ...
    template<typename T>
    constexpr MPL::EnableIfT<Detail::IsFieldLocation<T>::value, Detail::Reset<T>> reset(T) {
        static_assert(
          Detail::IsSetToClear<T>::value,
          "Access violation: Register::reset only works on set to clear bits");
        return {};
    }
